...
echo "  Wordpress ..."
mysqldump --no-tablespaces -u $mysql_user --password="$mysql_password" $mysql_database_wordpress | gzip > $DIRNAME/wp.sql.gz
if [[ -n $mysql_database_civicrm ]]; then
  echo "  CiviCRM..."
  mysqldump --no-tablespaces -u $mysql_user --password="$mysql_password" $mysql_database_civicrm | gzip > $DIRNAME/civicrm.sql.gz
fi
